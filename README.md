...
The original V0 scope excluded AI, accounts, sync, and cloud services. The user has since authorized the optional Phase 6 Ask slice described below; accounts, sync, and autonomous execution remain deferred. Local history completion (Phase 4) is in.
...
Phase 5 next-command prediction is in the tree. Session chrome uses a native split view, 18pt type, and Ghostty macOS glass. Optional AI Ask supports **OpenAI API, Codex, Anthropic API, Vercel AI Gateway, and Grok (xAI)**. At a ready shell prompt, type a clear conversational request such as `Help me find the largest files` and press Return. Sora marks it as **AI prompt** while typing and opens Ask inside the same terminal tab. Press Cmd+Return to run a detected sentence as shell input, or start with `/agent ` to force a command-like question to AI. **AI → Ask Sora** (Cmd+Shift+A) opens the same inline panel directly. Select a provider and enable AI in Setup. Save an API key to Keychain, or use Codex's ChatGPT sign-in with an installed Codex 0.153.0+ CLI/app. Stop cancels. **Attach webpage** fetches a public HTML/text page, shows the exact static text snapshot for review, and attaches it to the next question. Each provider keeps its own conversation and model setting. Generated commands are displayed and never executed. See [`docs/ai-ask.md`](docs/ai-ask.md).
