...
final class WebpageTests: XCTestCase {
    func testURLValidationAndNormalization() throws {
        XCTAssertEqual(try WebpageFetcher.url(" example.com/#about ").absoluteString, "https://example.com/")
    ...
    func testStaticHTMLExtractionNeverIncludesScriptsStylesOrAttributes() throws {
        let html = """
        <!doctype html><html><head><title>Example &amp; Work</title><style>body { color:red }</style></head>
        ...
        XCTAssertEqual(page.title, "Example & Work")
