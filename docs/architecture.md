...
- session chrome: flush two-column `HStack` (sidebar | terminal), hidden window title/toolbar. No `NavigationSplitView` (Tahoe draws that as floating rounded cards). Window frost is a square `NSGlassEffectView` behind both columns.
