...
- **`ghostty_surface_ime_point` is top-left origin.** Convert with `y = viewHeight - imeY` for AppKit overlays. `x` is the cursor cell midpoint — subtract half a cell width so ghost text starts on the leading edge. Use CELL_SIZE rescaled into IME-height space for both that offset and the per-column pitch. Pin `ghostty_surface_quicklook_font` to the configured 18pt size. Draw one CoreText glyph per cell, left-aligned (no extra centering). Baseline matches Ghostty's face-centered `cell_baseline` after `adjust-cell-height`. Prefix ghost text is suppressed while `GHOSTTY_ACTION_SCROLLBAR` reports the viewport away from the live prompt. Next-command prediction renders in a sticky sibling footer under the grid (not as a Metal overlay). Do not replace libghostty's layer.
