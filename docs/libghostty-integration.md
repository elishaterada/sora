...
- **`ghostty_surface_ime_point` is top-left origin.** Convert with `y = viewHeight - imeY` for AppKit overlays. `x` is the cursor cell midpoint — subtract half a cell width so ghost text starts on the leading edge. Prefer CELL_SIZE width when its height matches the IME height; only rescale across unit mismatches, and fall back to the font advance if rescale is ~2× off (a points/pixels mix-up that hides a leading space under the last typed glyph). Pin `ghostty_surface_quicklook_font` to the configured 18pt size. Draw one CoreText glyph per cell, left-aligned (no extra centering). Baseline matches Ghostty's face-centered `cell_baseline` after `adjust-cell-height`. Prefix ghost text is suppressed while `GHOSTTY_ACTION_SCROLLBAR` reports the viewport away from the live prompt. Next-command prediction renders in a sticky sibling footer under the grid (not as a Metal overlay). Do not replace libghostty's layer.
