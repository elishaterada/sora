...
- **`ghostty_surface_ime_point` is top-left origin.** Convert with `y = viewHeight - imeY` for AppKit overlays. Use `x` as the ghost leading edge the same way Ghostty's AppKit IME rect does, so a leading space in the suggestion (` -lah`, ` status`) occupies a real cell after the typed prefix. Prefer CELL_SIZE width when its height matches the IME height; only rescale across unit mismatches, and fall back to the font advance if rescale is ~2× off. Pin `ghostty_surface_quicklook_font` to the configured 18pt size. Draw one CoreText glyph per cell, left-aligned, including the leading space character. Baseline matches Ghostty's face-centered `cell_baseline` after `adjust-cell-height`. Prefix ghost text is suppressed while `GHOSTTY_ACTION_SCROLLBAR` reports the viewport away from the live prompt. Next-command prediction renders in a sticky sibling footer under the grid. Do not replace libghostty's layer.
