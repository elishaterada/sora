...
- **`ghostty_surface_ime_point` is top-left origin.** Convert with `y = viewHeight - imeY` for AppKit overlays. `x` is the cursor cell midpoint — subtract half a cell width so ghost text starts on the leading edge. Cell width comes from the terminal font advance (with CELL_SIZE rescaled into IME-height space as a check). Pin `ghostty_surface_quicklook_font` to the configured 18pt size — embedded builds can return a content-scaled face that breaks advance/baseline. Draw with AppKit string drawing on that face. Do not replace libghostty's layer.
