...
- **`ghostty_surface_ime_point` is top-left origin.** Convert with `y = viewHeight - imeY` for AppKit overlays. `x` is the cursor cell midpoint — subtract half a cell width so ghost text starts on the leading edge. Use `ghostty_surface_quicklook_font` (CTFont) and Ghostty's cell-baseline centering so the overlay matches the grid under `adjust-cell-height`. Do not replace libghostty's layer.
