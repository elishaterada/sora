...
- **`ghostty_surface_ime_point` is top-left origin.** Convert with `y = viewHeight - imeY` for AppKit overlays. `x` is the cursor cell midpoint — subtract half a CELL_SIZE width so ghost text starts on the leading edge and shares columns with typed glyphs. Refresh ghost placement from libghostty's wakeup after PTY echo advances the cursor; keyDown alone is too early and leaves suggestions one cell left of the caret. Prefer CELL_SIZE width for pitch. Keep leading spaces in suggestion text (` -lah`). Pin `ghostty_surface_quicklook_font` to the configured 18pt size. Draw one CoreText glyph per cell, left-aligned. Baseline matches Ghostty's face-centered `cell_baseline` after `adjust-cell-height`. Prefix ghost text uses a fixed prompt-start X (so echo flicker cannot pull the suffix left) and the **live IME Y** every refresh (so resize/layout cannot leave it floating mid-grid). Prefix ghost text is suppressed while `GHOSTTY_ACTION_SCROLLBAR` reports the viewport away from the live prompt. Next-command prediction renders in a sticky sibling footer under the grid. Do not replace libghostty's layer.
